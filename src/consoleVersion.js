...
class Game {
  ...
  choose() {
    // Asking the player which category they would like to play in
    if (Object.keys(this.score).length == 0) {
      // First time welcoming message
      console.log(
        chalk.blue(
          `Hey there! Welcome to Shrugman, I am Shruggy, the last person you want to see when playing this game! The goal of the game is to guess the correct title of either a movie or a book, one letter at a time. Any wrong guesses will cause me to reveal a tiny bit more of myself! And once you see all of me, then you'll know that ${chalk.bgRed.bold(
            "THE END IS NEIGH"
          )}. Anyways, best of luck, enjoy, or something like that, ${chalk.red(
            "I don't really care."
          )}`
        ),
        "\n"
    ...
    } else if (Object.keys(this.score).length == 1) {
      // Second game message
      console.log(
        "Shruggy:",
        chalk.blue(
          "I'm sorry, did my snarky comment hurt your feelings? Here, let me do something to help with that. (Shrugs)"
        ),
        "\n"
      );
    } else {
      ...
      let message =
        this.snarkyComments[
          Math.floor(Math.random() * this.snarkyComments.length)
        ];
      console.log("Shruggy:", chalk.blue(message), "\n");
    }
    let guessedLength = {
      // Keeping track of the number of movie and book titles guessed so far
      movies: 0,
      books: 0,
    };
    for (let n in this.score) {
      // Going through the score to increment books and movies
    ...
    do {
      ...
      if (!this.options.hasOwnProperty(choice))
        ...
        console.log(
          `Sorry, but you have exhausted the entire ${choice} database! Come back in 20 years, maybe it'll be updated then!`
        );
      } else {
  ...
  randomPicker() {
    // Returns a random element from the category choosen as the title to be guessed
    if (this.category == "books") {
      do {
        this.title =
          this.options["books"][
            Math.floor(Math.random() * this.options["books"].length)
          ];
      ...
      do {
        this.title =
          this.options["movies"][
            Math.floor(Math.random() * this.options["movies"].length)
          ];
      } while (this.score.hasOwnProperty(this.title)); // Making sure a title isn't presented twice
  ...
  creatingUnderscoredTitle() {
    // Creating a masked version of the title that reveals itself bit by bit when correct letters are guessed
    let maskedTitle = "";
    for (let i = 0; i < this.title.length; i++) {
      if (this.title[i] == " ") maskedTitle += this.title[i];
      else maskedTitle += "_";
    }
  ...
  startGame() {
    // The method that takes an input from the player, and passes it into the main method for checking
    ...
    if (this.wrongGuesses.length == 10) {
      let message =
        this.losingMessage[
          Math.floor(Math.random() * this.losingMessage.length)
        ];
      console.log("Shruggy:", chalk.red(message), "\n");
      console.log(
        chalk.white.bgGreen("Answer: ", chalk.bold(this.title)),
        "\n"
      );
      ...
      let message =
        this.winningMessage[
          Math.floor(Math.random() * this.winningMessage.length)
        ];
      console.log("Shruggy:", chalk.blue(message), "\n");
  ...
  shrugman(guess) {
    ...
    if (guess.length !== 1) {
      console.log(
        chalk.red(
          guess == 0 ? "Input is needed" : "Only type in one letter at a time!",
          "\n"
        )
      );
      this.displayInfo();
    ...
    if (Object.keys(this.guessedLetters).includes(guess.toLowerCase())) {
      ...
      for (let i = 0; i < this.maskedTitle.length; i++) {
        if (guess.toLowerCase() == this.title[i].toLowerCase())
          this.maskedTitle =
            this.maskedTitle.slice(0, i) +
            this.title[i] +
            this.maskedTitle.slice(i + 1);
        this.guessedLetters[guess.toLowerCase()] = "right";
      }
    } else {
      this.wrongGuesses += this.shrug.slice(
        this.wrongGuesses.length,
        this.wrongGuesses.length + 1
      );
  ...
  printScore() {
    // Prints the results of all the games played so far
    let num = 1;
    ...
    for (let i in this.score) {
      console.log(
        `${num}. ${i}: ${
          this.score[i] == "win"
            ? chalk.blue(this.score[i])
            : chalk.red(this.score[i])
        }`
      );
      num++;
    }
  ...
  playAgain() {
    // Asks the player if another game is desired
    let n;
    do {
      ...
      if (n == "yes" || n == "y") {
        ...
        this.reset()
          ...
          .startGame()
          .printScore()
          .playAgain(); // Starting an another chain of methods
      } else if (n == "no" || n == "n") {
  ...
  reset() {
    // Resetting the board for the next game
